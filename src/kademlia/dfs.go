...
func (k *Kademlia) CreateFile(cfReq CreateFileRequest, cfRes *CreateFileResult) {
    ...
    if len(fvRes.Nodes) > 0 {
        ...
        fileContent.Content = cfReq.Content
        b = new(bytes.Buffer)
        gob.NewEncoder(b).Encode(fileContent)
        fileBlockValue := b.Bytes()
        fileBlockKey, err := FromBytes(fileBlockValue)
        if err != nil {
            cfRes.Err = err
            return
        }
        ...
        fileInodeKey, err := FromBytes(fileInodeValue)
        if err != nil {
            cfRes.Err = err
            return
        }
        ...
        dirInodeKey, err := FromBytes(dirInodeValue)
        if err != nil {
            cfRes.Err = err
            return
        }
...
func (k *Kademlia) CreateDir(cdReq CreateDirRequest, cdRes CreateDirResult) {
    ...
    if len(fvRes.Nodes) > 0 {
        ...
        upperDirInodeKey, err := FromBytes(upperDirInodeValue)
        if err != nil {
            cdRes.Err = err
            return
        }
...
func (k *Kademlia) FindFile(req FindFileRequest, res *FindFileResult) {
    ...
    dirPath := strings.Join(l[:len(l)-1], "/")
    fileName := l[len(l)-1]
